...
exports.fakePredict = async () => {
  try {
    for await (a of listNameDeviceAndGPS) {
      ...
      const data = {
        ...
        GPS_Lon: 0,
        Predict_date1: date1,
        Predict_value1: Math.floor(Math.random() * 400),
        Predict_date2: date2,
        Predict_value2: Math.floor(Math.random() * 400),
        Predict_date3: date3,
        Predict_value3: Math.floor(Math.random() * 400),
      };
      const predicts = [
        {
          Date: new Date(data.Predict_date1),
          value: data.Predict_value1,
        },
        {
          Date: new Date(data.Predict_date2),
          value: data.Predict_value2,
        },
        {
          Date: new Date(data.Predict_date3),
          value: data.Predict_value3,
        },
      ];
      let station = await Station.findOne({ NameDevice: data.NameDevice });
